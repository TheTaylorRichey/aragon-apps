...
contract HybridRepresentativeProxy {
    ...
    function _canVote(Voting _voting, uint256 _voteId, address _voter) internal returns (bool) {
        return _isWhitelisted(_voter) && _voting.canVote(_voteId, _voter);
    }

    function _isProxyVoting(address _principal) internal returns (bool) {
        return proxyVotingRegistry.isValidProxyVoting(_principal);
    }

    function _isWhitelisted(address _principal) internal returns (bool) {
...
contract ProxyVotingRegistry {
    ...
    function isValidProxyVoting(address _proxyVoting) public returns (bool) {
        return proxyVotings[_proxyVoting];
    }

    function isValidRepresentativeProxy(address _representativeProxy) public returns (bool) {
